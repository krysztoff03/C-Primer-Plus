...
int main()
{
    float milesTraveled, gallonsConsumed;
	printf("Enter the number of miles traveled:");
	scanf("%f", &milesTraveled);
	printf("Enter the number of gallons consumed:");
	scanf("%f", &gallonsConsumed);
	float milesPerGallon = milesTraveled / gallonsConsumed;
	float liters, km;
	liters = gallonsConsumed * GTOL;
	km = milesTraveled * MTOKM;
	printf("Liters consumed per 100km: %.1f\n", (100 * liters) / km);
    return 0;
}
